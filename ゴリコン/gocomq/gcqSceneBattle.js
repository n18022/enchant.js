  ...
  initialize: function() {
          ...
          if(e == 4) {
            ...
            if(engine.girls[2].exist) {
              ...
              if(girlIcons[2].firstChild.frame != 28)
                mes.push({
                  getMessage: function() {
                    return [engine.girls[2].name + " 達は覚醒した", "　"];
                  }, type: ActType.Evolve, to: 0
                });
    ...
    var messageArea = new MessageArea(engine, function(actResult) {
      ...
      if(actResult.type == ActType.Love && actResult.success) {
        ...
        girlIcons[0].firstChild.frame = 28;
        girlIcons[1].firstChild.frame = 28;
        girlIcons[2].firstChild.frame = 28;
        girlIcons[3].firstChild.frame = 28;
        effect();
      }
      ...
      if(!engine.boys[0].exist) {
        ...
        if(scene.actType == 4 && engine.girls[2].exist)
          dummyList.updateTexts([engine.girls[2].name, "「私達を怒らせない方が良い」"]);
        else
